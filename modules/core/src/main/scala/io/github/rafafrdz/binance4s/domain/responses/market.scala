...
object OrderBookEntry:
  ...
  given Encoder[OrderBookEntry] = Encoder.instance { e =>
    Json.arr(Json.fromString(e.price), Json.fromString(e.qty))
  }
...
object Kline:
  given Decoder[Kline] = Decoder.instance { c =>
    ...
    yield Kline(openTime, open, high, low, close, volume, closeTime, quoteAssetVolume, numberOfTrades, takerBuyBaseAssetVolume, takerBuyQuoteAssetVolume)
  }
  given Encoder[Kline] = Encoder.instance { k =>
    Json.arr(
      Json.fromLong(k.openTime), Json.fromString(k.open), Json.fromString(k.high),
      Json.fromString(k.low), Json.fromString(k.close), Json.fromString(k.volume),
      Json.fromLong(k.closeTime), Json.fromString(k.quoteAssetVolume),
      Json.fromInt(k.numberOfTrades), Json.fromString(k.takerBuyBaseAssetVolume),
